/*
NOTE: 
- 8/Sep/2022 Sua code NTP cho chuan @@
- 19/Aug/2022  them code cho dong ho sua chua! RB
...
#include "socket.h"
const time_t built_time = 1662625855;
time_t timenow;
...
{//=> Ban tin GPS: $GPS034007060819AA10	
	...
	if((USART1_rx_data_buff[6] ==' ')&(USART1_rx_data_buff[13] ==' '))
	{
		//093241 020822 V
		days 		= 10*convert_atoi(USART1_rx_data_buff[7])+convert_atoi(USART1_rx_data_buff[8]);
		if(days > 31) return;
		months 	= 10*convert_atoi(USART1_rx_data_buff[9])+convert_atoi(USART1_rx_data_buff[10]);
		if(months > 12) return;
		years 	= 10*convert_atoi(USART1_rx_data_buff[11])+convert_atoi(USART1_rx_data_buff[12]);
		
		hours 	= 10*convert_atoi(USART1_rx_data_buff[0])+convert_atoi(USART1_rx_data_buff[1])  ;//UTC
		if(hours > 23) return;
		minutes = 10*convert_atoi(USART1_rx_data_buff[2])+convert_atoi(USART1_rx_data_buff[3]);
		if(minutes > 59) return;
		seconds = 10*convert_atoi(USART1_rx_data_buff[4])+convert_atoi(USART1_rx_data_buff[5]);
		...
		//Update SNMP data table
		if(USART1_rx_data_buff[14]=='A') gps1_stt = 1;
		else gps1_stt = 0;
		if(USART1_rx_data_buff[14]=='A') gps2_stt = 1;
		else gps2_stt = 0;
		if(USART1_rx_data_buff[15]=='1') power1_stt = 1;
		else power1_stt = 0;
		if(USART1_rx_data_buff[16]=='1') power2_stt = 1;
		else power2_stt = 0;
	}
}
//Xu ly ban tin GPS
