...
#endif
#ifdef _HTTPSERVER_DEBUG_
						printf("> HTTPSocket[%d] : End of parsing HTTP type, STATE_HTTP_REQ_DONE\r\n", s);
#endif
...
#ifdef _HTTPSERVER_DEBUG_
			printf("> HTTPSocket[%d] : HTTP Response body - send len [ %u ]byte\r\n", s, send_len);
#endif
	}
...
// ## 20141219 Eric added, for 'File object structure' (fs) allocation reduced (8 -> 1)
...
#ifdef _HTTPSERVER_DEBUG_
	printf("> HTTPSocket[%d] : [Send] HTTP Response body [ %u ]byte\r\n", s, send_len);
#endif
	...
	else
	{
		HTTPSock_Status[get_seqnum].file_offset += send_len;
#ifdef _HTTPSERVER_DEBUG_
		printf("> HTTPSocket[%d] : HTTP Response body - offset [ %u ]\r\n", s, HTTPSock_Status[get_seqnum].file_offset);
#endif
	}
...
#endif
	send_len = sprintf((char *)buf, "%s%d\r\n\r\n%s", RES_JSONHEAD_OK, file_len, http_body);
#ifdef _HTTPSERVER_DEBUG_
	...
	switch (p_http_request->METHOD)
	{
		...
		case METHOD_GET :
			...
			else if(p_http_request->TYPE == PTYPE_JSON)
			{
				...
				content_found = http_get_json_handler(uri_name, pHTTP_TX, &file_len);
				if(content_found && (file_len <= (DATA_BUF_SIZE-(strlen(RES_CGIHEAD_OK)+8))))
				{
					send_http_response_json(s, http_response, pHTTP_TX, (uint16_t)file_len);
				}
				else
				{
			...
			{// If No CGI request, Try to find The requested web content in storage (e.g., 'SD card' or 'Data flash')
				...
				if((fr = f_open(&fs, (const char *)uri_name, FA_READ)) == 0)
				{
					...
					file_len = fs.obj.objsize;
					content_addr = fs.obj.sclust;
					HTTPSock_Status[get_seqnum].storage_type = SDCARD;
				}
#elif _USE_FLASH_
				else if(/* Read content from Dataflash */)//sua cai nay thanh cai gi nhi
				{
					content_found = 1;
					HTTPSock_Status[get_seqnum]->storage_type = DATAFLASH;
					; // To do
					file_len = fs.obj.objsize;// sua cai nay thanh kich thuoc
					content_addr = fs.obj.sclust;//sua cai nay thanh dia chi cua trang web
				}
#endif
				else
				{
				...
				{
#ifdef _HTTPSERVER_DEBUG_
					printf("> HTTPSocket[%d] : Requested content len = [ %u ]byte\r\n", s, file_len);
#endif
...
uint8_t display_reg_webContent_list(void)
{
	...
	else
	{
		...
		for(i = 0; i < total_content_cnt; i++)
		{
			...
			printf("%u byte, ", web_content[i].content_len);
